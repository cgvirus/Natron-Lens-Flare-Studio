...
//parametres
uniform float Radius = .5; // Radius, min=0., max=2.
uniform float Thickness = .3; // Thickness, min=0., max=1.5
uniform float softness = .8; // Softness, min=0., max=1.
uniform int raycount = 3; // Ray Count , min=0., max=200.
uniform float raydepth = 1.1; // Ray depth , min=-1.5, max=1.5
